...
def _parse_create(sql):
    ...
    # Use top-level comma splitting to keep UNIQUE(event_id, buyer_name) intact
    for col_def in _split_top_level(cols_def):
        col_def = col_def.strip()

        # Detect composite UNIQUE constraint
        ...
        # Otherwise treat as column definition
        parts = col_def.split()
        if len(parts) < 2:
        ...
        # Handle "PRIMARY KEY" appearing as two tokens
...
def _parse_select(sql):
    ...
    # Fallback: simple SELECT * FROM table [WHERE ...]
...
def _parse_update(sql):
    # Normalize spacing around SET and WHERE to make regex simpler
    ...
    # Split set_clause into assignments at top-level commas
    ...
    for a in assignments:
        ...
        col = col.strip()
        val = val.strip()
        set_map[col] = _strip_quotes(val)

    # Parse simple WHERE of form col = value (value may be quoted)
...
def _tokenize_top_level(s: str, sep=","):
    """Yield top-level parts split by sep, ignoring sep inside single quotes or parentheses."""
    ...
    while i < len(s):
        ...
        if ch == "'" and not in_quotes:
            ...
            buf.append(ch)
            i += 1
            # consume until closing quote, handling doubled single quotes
            while i < len(s):
                buf.append(s[i])
                if s[i] == "'" and i + 1 < len(s) and s[i+1] == "'":
                    # doubled quote -> include both and skip next
                    buf.append(s[i+1])
                    i += 2
                    continue
                if s[i] == "'":
...
def _strip_quotes(val: str):
    """If val is single-quoted, unquote and unescape doubled single quotes."""
    val = val.strip()
